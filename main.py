...
class Preprocessing:
    ...
    def merge_channel(self, conn, id, task_param):
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute("UPDATE avt_tasks SET task_stat = 'finished', task_output = %s WHERE id = %s", (ftp_dir, id,))
        except ftplib.all_errors as e:
            cursor = conn.cursor()
            cursor.execute("UPDATE avt_tasks SET task_stat = 'error' WHERE id = %s", (id,))
            print(f"FTP error: {e}")
    ...
    def sharpen_image(self, conn, id, task_param):
        ...
        try:
            ...
            print("Connection closed")
            cursor = conn.cursor()
            cursor.execute("UPDATE avt_tasks SET task_stat = 'finished', task_output = %s WHERE id = %s", (save_dir, id,))
        except ftplib.all_errors as e:
            cursor = conn.cursor()
            cursor.execute("UPDATE avt_tasks SET task_stat = 'error' WHERE id = %s", (id,))
            print(f"FTP error: {e}")
