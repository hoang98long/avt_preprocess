...
class Preprocessing_Image:
    ...
    def radiometric_correction(self, input_path, output_path, png_paths):
        ...
        if num_channels == 1:  # Ảnh đơn sắc
            single_channel_img = tiff_img[0]
            adjusted_single_channel = match_histograms(
                single_channel_img,
        ...
        elif num_channels == 3:  # Ảnh RGB không có kênh IR
            rgb_channels = tiff_img
            png_histograms = []
            ...
            with rasterio.open(output_path, 'w', **profile) as tiff_dst:
                tiff_dst.write(adjusted_rgb)

        elif num_channels >= 4:  # Ảnh RGB có kênh IR
            ...
            raise ValueError("Unsupported number of channels in the input TIFF image.")
